...
ligand_1_sys = bss.IO.readMolecules([f"{ligand_path}lig_h_{sys.argv[1] + 1}_ligand_prepped.rst7",
                                     f"{ligand_path}lig_h_{sys.argv[1] + 1}_ligand_prepped.prm7"])
ligand_2_sys = bss.IO.readMolecules([f"{ligand_path}lig_h_{sys.argv[2] + 1}_ligand_prepped.rst7",
                                     f"{ligand_path}lig_h_{sys.argv[2] + 1}_ligand_prepped.prm7"])
...
system_1 = bss.IO.readMolecules(
    [f"{protein_path}lig_h_{sys.argv[1] + 1}_system_prepped.rst7",
     f"{protein_path}lig_h_{sys.argv[1] + 1}_system_prepped.prm7"])
system_2 = bss.IO.readMolecules(
    [f"{protein_path}lig_h_{sys.argv[2] + 1}_system_prepped.rst7",
     f"{protein_path}lig_h_{sys.argv[2] + 1}_system_prepped.prm7"])
...
with open("network.dat", "r") as lambdas_file:
    ...
    for row in reader:
        if row[0] == sys.argv[1] and row[1] == sys.argv[2]:
            n_windows = int(row[2])
if not n_windows:
    raise NameError(f"The perturbation {sys.argv[1]}~{sys.argv[2]} was not found in network.dat.")
...
workdir = f"../outputs/{engine}/{sys.argv[1]}~{sys.argv[2]}/"
